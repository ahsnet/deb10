...
# set repo
wget -O /etc/apt/sources.list "https://raw.githubusercontent.com/ahsnet/deb10/master/repo"
...
wget -O /etc/nginx/nginx.conf "https://raw.githubusercontent.com/ahsnet/deb10/master/nginx.conf"
mkdir -p /home/vps/public_html
echo "<pre>www.AHSNET.xyz</pre>" > /home/vps/public_html/index.html
wget -O /etc/nginx/conf.d/vps.conf "https://raw.githubusercontent.com/ahsnet/deb10/master/vps.conf"
...
sed -i '/Port 22/a Port 333' /etc/ssh/sshd_config
service ssh restart

# install dropbear
apt-get -y install dropbear
sed -i 's/NO_START=1/NO_START=0/g' /etc/default/dropbear
sed -i 's/DROPBEAR_PORT=22/DROPBEAR_PORT=400/g' /etc/default/dropbear
...
wget -O bersih "https://raw.githubusercontent.com/ahsnet/script/master/delexp.sh"
...
echo "-------"  | tee -a log-install.txt
echo "OpenSSH  : 22, 333"  | tee -a log-install.txt
echo "Dropbear : 400, 444"  | tee -a log-install.txt
echo "SSL      : 443"  | tee -a log-install.txt
echo "Squid3   : 80, 8080, 3128 (limit to IP SSH)"  | tee -a log-install.txt
echo "OpenVPN  : TCP 1194 (client config : http://$MYIP:81/client.ovpn)"  | tee -a log-install.txt
echo "badvpn   : badvpn-udpgw port 7000-7900"  | tee -a log-install.txt
